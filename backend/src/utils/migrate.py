...
if TYPE_CHECKING:
    from providers.vectorstore import BaseVectorStoreProvider, ChromaProvider, QdrantProvider
    ...
    def migrate(
        ...
        for idx, collection_name in enumerate(collections):
            if progress_callback:
                progress_callback(
                    idx, total_collections,
                    f"Migrating collection: {collection_name}"
                )
            
            collection_result = self._migrate_collection(
    ...
    def _migrate_collection(
        ...
        try:
            ...
            while offset < source_count:
                ...
                if progress_callback:
                    progress_callback(
                        min(offset, source_count), source_count,
        ...
        for collection_name in collections:
            try:
                ...
                if source_count != target_count:
                    ...
                    logger.debug(
                        f"Verified '{collection_name}': {target_count} documents"
                    )
                    
            except Exception as e:
                warnings.append(
                    f"Could not verify '{collection_name}': {e}"
                )
