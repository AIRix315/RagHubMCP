...
class ChromaConfig(BaseModel):
    ...
    persist_dir: str | None = Field(default="./data/chroma", description="Persistence directory")
    host: str | None = Field(default=None, description="Remote host (None for local)")
    port: int | None = Field(default=None, description="Remote port (None for local)")
    ...
    @field_validator("port")
    @classmethod
    def validate_remote_port(cls, v: int | None) -> int | None:
...
class ProviderInstance(BaseModel):
    ...
    dimension: int | None = Field(default=None, ge=1, description="Embedding dimension")
    ...
    def validate_default_exists(self) -> ProviderCategory:
        ...
        if self.default and self.instances:
            ...
            if self.default not in instance_names:
                raise ValueError(
                    f"Default provider '{self.default}' not found in instances: {instance_names}"
                )
        return self
...
class IndexerConfig(BaseModel):
    ...
    chunk_size: int = Field(
        default=500, ge=50, le=10000, description="Max characters per chunk (50-10000)"
    )
    chunk_overlap: int = Field(
        default=50, ge=0, le=5000, description="Overlap between chunks (0-5000)"
    )
    max_file_size: int = Field(
        default=1048576, ge=1, le=104857600, description="Max file size in bytes (1-100MB)"
    )
...
class MCPValidationConfig:
    ...
    def validate_int_range(
        cls, value: Any, field_name: str, min_val: int, max_val: int
    ) -> str | None:
...
class PipelineProfileConfig(BaseModel):
    ...
    retrieval_multiplier: float = Field(
        default=2.0, ge=1.0, le=5.0, description="Multiplier for initial retrieval"
    )
    alpha: float = Field(
        default=0.5, ge=0.0, le=1.0, description="Vector search weight in hybrid retrieval"
    )
    beta: float = Field(
        default=0.5, ge=0.0, le=1.0, description="BM25 search weight in hybrid retrieval"
    )
    rrf_k: int = Field(
        default=60, ge=1, le=1000, description="RRF constant for reciprocal rank fusion"
    )
...
class ProviderDefaultsConfig(BaseModel):
    ...
    embedding_timeout: float = Field(
        default=60.0, ge=1.0, description="Timeout for embedding API calls in seconds"
    )
    llm_max_tokens: int = Field(default=1024, ge=1, description="Maximum tokens for LLM responses")
    rerank_max_length: int = Field(
        default=512, ge=128, description="Maximum token length for reranking"
    )
    default_embedding_dimension: int = Field(
        default=768, ge=1, description="Default embedding dimension"
    )
...
class PathConfig(BaseModel):
    ...
    chroma_persist_dir: str = Field(
        default="./data/chroma", description="Chroma database persistence directory"
    )
    qdrant_path: str = Field(default="./data/qdrant", description="Qdrant database path")
    rerank_cache_dir: str = Field(
        default="./data/flashrank_cache", description="Rerank model cache directory"
    )
    bm25_persist_dir: str = Field(
        default="./data/bm25", description="BM25 index persistence directory"
    )
...
def _parse_provider_category(data: dict[str, Any]) -> ProviderCategory:
    ...
    return ProviderCategory(default=data.get("default", ""), instances=data.get("instances", []))
