...
class TestVectorRetrieverWithProvider:
    ...
    async def test_retrieve_returns_documents(self):
        ...
        mock_provider = MagicMock()
        mock_provider.query.return_value = QueryResult(results=[
            SearchResult(id="doc-1", document="Document 1", metadata={"source": "file1.py"}, score=0.1),
            SearchResult(id="doc-2", document="Document 2", metadata={"source": "file2.py"}, score=0.2),
        ])
    ...
    async def test_retrieve_converts_distance_to_score(self):
        ...
        mock_provider.query.return_value = QueryResult(results=[
            SearchResult(id="1", document="Close match", metadata={}, score=0.0),  # distance=0 means perfect match
