...
class Indexer:
    ...
    def _index_chunks(self, chunks: list[Chunk], source_path: Path) -> None:
        ...
        ids = [
            f"{source_path}:{chunk.start}:{chunk.end}"
            for chunk in chunks
        ]
    ...
    def index_chunks(
        ...
        for i in range(0, len(chunks), batch_size):
            ...
            # Generate unique IDs
            ids = [
                f"chunk_{i + j}:{chunk.start}:{chunk.end}"
        ...
        logger.info(f"Indexed {len(chunks)} chunks in {(len(chunks) + batch_size - 1) // batch_size} batches")
    ...
    def index_directory(
        ...
        if on_progress:
            on_progress(total, total, f"Completed: {result.files_indexed} files, {result.chunks_created} chunks")
