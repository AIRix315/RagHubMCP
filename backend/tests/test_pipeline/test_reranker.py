...
from pipeline.reranker import Reranker, PipelineReranker, NoOpReranker, FallbackReranker
...
class TestRerankerABC:
    ...
    def test_reranker_name_property(self):
        ...
        class TestReranker(Reranker):
            async def rerank(self, query: str, documents: list[Document], options: dict = None) -> list[Document]:
                return documents
...
class TestPipelineReranker:
    ...
    async def test_rerank_respects_top_k_option(self):
        ...
        docs = [
            Document(id=str(i), text=f"Document {i}", score=0.9 - i * 0.1)
            for i in range(10)
        ]
    ...
    async def test_rerank_preserves_metadata(self):
        ...
        docs = [
            Document(id="1", text="Document", score=0.9, metadata={"source": "test", "author": "user"}),
        ]
...
class TestNoOpReranker:
    ...
    async def test_rerank_respects_topk_option(self):
        ...
        docs = [
            Document(id=str(i), text=f"Doc {i}", score=0.9)
            for i in range(10)
        ]
...
class TestFallbackReranker:
    ...
    async def test_rerank_uses_primary_on_success(self):
        ...
        mock_primary.rerank = AsyncMock(return_value=[
