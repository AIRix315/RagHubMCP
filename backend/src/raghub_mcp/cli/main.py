...
@click.option(
    "--output", "-o", default="table", type=click.Choice(["table", "json"]), help="Output format"
)
@pass_context
...
def _display_providers(
    data: list[dict[str, Any]] | dict[str, Any], provider_type: str | None
) -> None:
...
def _display_provider_table(providers: list[dict[str, Any]]) -> None:
    ...
    for p in providers:
        ...
        status_style = (
            "green" if status == "active" else "yellow" if status == "inactive" else "red"
        )
...
def provider_test(ctx: CLIContext, name: str, provider_type: str, query: str) -> None:
    ...
    try:
        ...
        if success:
            console.print("[green]✓ Test successful[/]")
...
def pipeline_debug(ctx: CLIContext, query: str, profile: str) -> None:
    ...
    """
    try:
        console.print("[bold blue]Pipeline Debug Mode[/]")
        console.print(f"[dim]Query: {query}[/]")
...
def _display_pipeline_debug(debug_info: dict[str, Any]) -> None:
    ...
    console.print("[bold]Pipeline Execution[/]")
    console.print(f"[dim]Query ID: {query_id}[/]\n")
    ...
    for stage in stages:
        ...
        status_icon = "✓" if status == "completed" else "✗" if status == "error" else "○"
        status_style = (
            "green" if status == "completed" else "red" if status == "error" else "yellow"
        )

        console.print(
            f"[{status_style}]{status_icon} Stage: {name}[/{status_style}] ({latency:.2f}ms)"
        )

        if metadata:
            console.print("  [dim]Metadata:[/]")
...
def status(ctx: CLIContext, output: str) -> None:
    ...
    try:
        ...
        if health_response.status_code != 200:
            console.print("[red]✗ Server not responding[/]")
            sys.exit(1)
