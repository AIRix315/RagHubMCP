...
class MarkdownChunker(ChunkerPlugin):
    ...
    SETEXT_HEADING_PATTERN = re.compile(r"^(.+?)\n([=-]{3,})$", re.MULTILINE)
                ...
                chunks.append(
                    Chunk(
                        ...
                        metadata=self._create_metadata(
                            metadata, len(chunks), heading=heading_text, heading_level=level
                        ),
                    )
                )
    ...
    def _split_large_section(
        ...
        while start < text_len:
            ...
            chunk_metadata = self._create_metadata(base_metadata, chunk_idx, large_section=True)
            if heading:
                chunk_metadata["heading"] = heading

            chunks.append(
                Chunk(
                    text=chunk_text, start=offset + start, end=offset + end, metadata=chunk_metadata
                )
            )
